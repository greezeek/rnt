...
class Heartbeat
{
    ...
    public static function getPeaks($graph) {
        ...
        for ($i = 0; $i < count($graph); $i++) {
            if($i > 0) {
                if(self::isExtr($graph[$i-1],$graph[$i],$graph[$i+1])) {
                    $peaks[] = $graph[$i];
                }
            }
        }
    ...
    public static function getBeat($workout_id)
    {

    }

    public static function getWorkout($workout_id = false)
    {
